...
for (i in 1:num.of.samples){
  ...
  s.mean <- mean(s.norm)
  s.var <-var(s.norm)
  data.temp <-data.frame(s.mean, s.var) 
  dat <- rbind(dat, data.temp)
}


#sample statistics value 
sample.mean.mean <- mean(dat$s.mean)
sample.var.mean <- mean(dat$s.var)
sample.mean.sd <- sd(dat$s.mean)
sample.var.sd <- sd(dat$s.var)

#Theoretical statistics value
theoretical.mean.mean <- 100
theoretical.mean.sd <- pop.sd/sqrt(sample.size)
theoretical.var.mean <- pop.sd^2
theoretical.var.sd <- sqrt(2*pop.sd^4/(sample.size - 1))

#comparision between the sample statistics and theoretical values
temp <- c(sample.mean.mean, sample.var.mean, sample.mean.sd, sample.var.sd,
          theoretical.mean.mean, theoretical.var.mean, theoretical.mean.sd, theoretical.var.sd)
comp <- matrix(temp, nrow=2,ncol=4, byrow=TRUE)
colnames(comp) <-c("mean_of_means", "mean_of_vars", "sd_of_means", "sd_of_vars" )
rownames(comp) <-c("sample", "theoretical")
comp
...
scatterplot2 <- ggplot(data=dat, aes(1:200, dat$s.var)) + geom_point() + geom_hline(yintercept = theoretical.var.mean, color="blue") + labs(title = "Sample var distribution", x="Sample Sets", y ="Sample var")
scatterplot2

#histogram of sample means
hist.mean <- ggplot(data=dat, aes(x=dat$s.mean)) + geom_histogram(binwidth=0.3, color="black", fill="wheat", alpha=.2) + labs(title = "Histogram of Sample Means", x = "Sample Mean", y="Frequency")
hist.mean

#histogram of sample var
hist.var <- ggplot(data=dat, aes(x=dat$s.var)) + geom_histogram(binwidth =11, color="black", fill="wheat", alpha=.2) + labs(title = "Histogram of Sample var", x = "Sample var", y="Frequency")
hist.var
...
Answer: 
When unrelative 20 dimentions exist, the Euler distances between points may change, which means, the chosen k nearest points will change to another k points. As in the previous 1 dimension case, the nearest points are truely nearest, the prediction should be nearest to the true y. when we come to the 21 dimension case, the predictor of the new k nearest may not be as good as the previous one. Then, the gaps between predictor and true y increased, and MSE increased.
...
ggplot(data = mse.opt, aes(1:20, mse.opt)) + geom_line()+ geom_point(color="red") + coord_cartesian(ylim = c(0,30))

```

The MSE shows an increasing trend overall, occasionally it will decrease. After all, the sample has its own occasionality. So line has some places that go down. But we can see that the global trend is increasing. 
